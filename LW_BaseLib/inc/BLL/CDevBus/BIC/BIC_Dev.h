...
#include "Bus.h"
#include "CMUX.h"
#if defined BIC_Commu_h && defined Bus_h
//------------------------------------------------------------------------------------------//
#ifndef BIC_Dev_h
#define BIC_Dev_h
#ifdef BIC_Dev_h
...
namespace BIC_ENV_DEV {
	...
	ExpandDevBusAttr*	GetEDA			(CMD_ENV* env);
	ExpandDevBusAttr*	GetEDA_M		(CMD_ENV* env);
	ExpandDevBusAttr*	GetEDA_A		(CMD_ENV* env);
	ExpandDevBusAttr*	GetEDA_S		(CMD_ENV* env);
	...
	CMD_SETVAR(SetEDA,			CMD_VID_EDA,			ExpandDevBusAttr*);
	CMD_SETVAR(SetEDA_M,		CMD_VID_EDA_M,			ExpandDevBusAttr*);
	CMD_SETVAR(SetEDA_A,		CMD_VID_EDA_A,			ExpandDevBusAttr*);
	CMD_SETVAR(SetEDA_S,		CMD_VID_EDA_S,			ExpandDevBusAttr*);
...
class BIC_CONNECT : public BIC_BASE{
	...
	public:
		static	bool32	SetConnectPar		(CMD_ENV* env,const STDSTR &par,uint32 devType);
		static	bool32	SetConnectPar2		(CMD_ENV* env,const STDSTR &par);
	public:
		static	uint8	PressAnyKey			(CMD_ENV* env,ExpandDevBusAttr* eda,SYS_TIME_S* dly);
		static	bool32	OnlineMode			(CMD_ENV* env,ExpandDevBusAttr* attr,CMD_TAIL tail);
	
		static	void	PrintTitle			(CMD_ENV* env,ExpandDevBusAttr* eda,bool32 blPrintTail);
};
//------------------------------------------------------------------------------------------//
