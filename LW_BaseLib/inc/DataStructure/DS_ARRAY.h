...
template <typename DS_T> class DS_ARRAY_FRAME{
	...
	public:
				 DS_ARRAY_FRAME(void);
				 DS_ARRAY_FRAME(uint32 size);
				 DS_ARRAY_FRAME(const DS_ARRAY_FRAME& C);
...
class ARRAY : public DS_ARRAY_FRAME<uint8>{
	...
	public:
		...
		const	STDSTR&	GetUnEscape	(STDSTR* _out,uint32 num);
		const	STDSTR&	ReadUnEscape(STDSTR* _out,uint32 num,uint32 offset = 0)const;
