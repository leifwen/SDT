...
IOSE CSSL_T1FD::ReadOneBlock(const ABORTCHECK& _ac,IOS* _ios,const UVOut &_out,uint32 bID,uint32 maxDlyMS,const uint32 maxTimes){
	...
	do{
		...
		if ((_ac.fun != nullptr) && (_ac.fun(_ac.p) != G_FALSE)){
			rcode = IOS_ERR;
			break;
		}
	}while(1);
...
void CSSL_TEST_PORT::Init(uint32 port,const ARRAY* _out,const ARRAY* _in,uint32 maxPackage,uint32 maxPages){
	...
	t0Thread.ThreadInit		(this,&CSSL_TEST_PORT::T0ThreadFun		,"T0");
	t1RxThread.ThreadInit	(this,&CSSL_TEST_PORT::T1RxThreadFun	,"T1Rx");
	t1TxThread.ThreadInit	(this,&CSSL_TEST_PORT::T1TxThreadFun	,"T1Tx");
...
//------------------------------------------------------------------------------------------//
bool32 CSSL_TEST_PORT::T0ThreadFun(void* p){
...
//------------------------------------------------------------------------------------------//
bool32 CSSL_TEST_PORT::T1RxThreadFun(void* strMD5){
	IOS		ios;
...
//------------------------------------------------------------------------------------------//
bool32 CSSL_TEST_PORT::T1TxThreadFun(void* strData){
	IOS		ios;
