...
void SCRIPT::Init(const DEVICE* dev){
	...
	if (dev != nullptr){
		...
		BIF_ENV_DEV::SetEDA(&cgENV,(ExpandDeviceAttr*)dev->EDA());
	}
	else{
...
bool32 SCRIPT::ExecuteCommand(COMMAND_NODE* tCommand,int32 frameTimeout){
	DEVICE		*device = BIF_ENV_DEV::GetEDA(&cgENV)->device;
	...
	if (tCommand->blEnableSend){
		...
		while(((i++ < maxCycle) || (maxCycle == 0)) && (IsTerminated() == G_FALSE)){
			...
			cgSBUF.array.Empty();
			device->PrintSendCommand((uint32)retStr.forSend.length(),retStr.forPrint);
			device->Send(retStr.forSend);
