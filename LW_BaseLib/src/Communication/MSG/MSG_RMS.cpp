...
//------------------------------------------------------------------------------------------//
MAPBSERVER::MAPBSERVER(uint32 rxSize,uint32 txSize,const ODEV_SYSTEM* logSys) : MAPBSERVER_BASE(rxSize,txSize,nullptr){
};
//------------------------------------------------------------------------------------------//
	...
	switch (mID){
		...
		case MESG_ANS_ListMServer:
			...
			*strP = strMesg;
			...
			break;
		case MESG_REQ_SetupDataChannel:
			blRet = OpenCoupleSocket(env,&rmssocket,&bsocket,msg,commu->Core()->GetOpenPar());
		...
		case MESG_REQ_Link:
			...
			blRet = G_FALSE;
			try{
				COMMU_NODE::LinkCouple(commu,bsocket);
