...
MEM_SSL::MEM_SSL(void) : COMMU_MEM(){
	t1DataRxThread.ThreadInit(this,&MEM_SSL::T1DataRxThreadFun	,"T1DRx");
...
void MEM_SSL::Init(const COMMU_TEAM* _team){
	...
	unitTeam->commu->ThreadAccept(&t1DataRxThread);
...
void MEM_SSL::Stop(void){
	...
	ELog(LogTitle(this,"MEM_SSL","Stop","::") << "IsCloseDueToOS() = "	<< unitTeam->commu->IsCloseDueToOS());
	...
	if (unitTeam->core->IsConnected() && (unitTeam->commu->IsCloseDueToOS() == G_FALSE)
...
//------------------------------------------------------------------------------------------//
uint32 MEM_SSL::Send(const ABORTCHECK& _ac,const UVIn&  _in){
	IOS	ios;
	IOS_clr(&ios);
	if (cgT1 != nullptr)
...
//------------------------------------------------------------------------------------------//
bool32 MEM_SSL::T1DataRxThreadFun(void* _team){
