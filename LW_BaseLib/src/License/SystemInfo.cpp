...
#define  DFP_GET_VERSION		(0x00074080)
	...
	for(UINT uDrive = 0;uDrive < 4;++ uDrive){
		...
		szDriveName += Str_ANSIToUnicode(Str_ToStr(uDrive)).c_str();
...
int32 WinNTHDSerialNumAsScsiReadWithAdminRights(BYTE* dwSerial,UINT* puSerialLen,UINT uMaxSerialLen){
	...
	for(int iController = 0;iController < 2;++ iController){
		...
		CString		szDriveName;
		szDriveName = _T("\\\\.\\Scsi");
		szDriveName += Str_ANSIToUnicode(Str_ToStr(iController)).c_str();
		szDriveName += _T(":");
...
int32 WinNTHDSerialNumAsPhysicalReadWithZeroRights(BYTE* dwSerial,UINT* puSerialLen,UINT uMaxSerialLen){
	...
	for (drive = 0; drive < 4; drive++){
		...
		CString		szDriveName;
		szDriveName = _T("\\\\.\\PhysicalDrive");
		szDriveName += Str_ANSIToUnicode(Str_ToStr(drive)).c_str();
...
void SI_GetNetCardInfo(STDSTR* stringData){
	...
	PIP_ADAPTER_INFO pIpAdapterInfo2 = pIpAdapterInfo;
	...
	if (nRel == ERROR_BUFFER_OVERFLOW){
		...
		pIpAdapterInfo2 = pIpAdapterInfo;
		nRel = GetAdaptersInfo(pIpAdapterInfo,&stSize);
	}
	if (nRel == ERROR_SUCCESS){
		...
		*stringData += "----------------------------------------------------------------------\n";
		while (pIpAdapterInfo){
			*stringData += "Number               : ";
			*stringData += Str_ToStr(++ netCardNum);
			*stringData += "\n";
			*stringData += "Name                 : ";
			...
			*stringData += "MAC Address          : ";
			for (DWORD i = 0; i < pIpAdapterInfo->AddressLength; i++){
				*stringData += Str_RTrim(Str_CharToHEX(&pIpAdapterInfo->Address[i],1,G_SPACE_OFF));
			...
			*stringData += "\n\n";
			...
			do{
				...
				*stringData += Str_ToStr(++IPnumPerNetCard);
				*stringData += "\n";
...
__int64 WinNTHDSerialNumAsPhysicalReadWithZeroRights(int driveNo,BYTE* dwSerial,UINT* puSerialLen,UINT uMaxSerialLen){
	...
	szDriveName = _T("\\\\.\\PhysicalDrive");
	szDriveName += Str_ANSIToUnicode(Str_ToStr(driveNo)).c_str();
...
const STDSTR& SystemInfo_GetHardiskInfo(STDSTR* stringData){
	...
	if (IsWindowsXPOrGreater()){
	//	return(*stringData);// Only Windows 2000, Windows XP, Windows Server 2003...
		uSystemInfoLen = 0;
...
const STDSTR& SystemInfo_GetHardiskInfo(STDSTR* retStr){
	*retStr += "HD1 is HD0001|";
	*retStr += "HD2 is HD0002|";
	*retStr += "HD3 is HD0003|";
	*retStr += "HD4 is HD0004|";
	return(*retStr);
};
//------------------------------------------------------------------------------------------//
const STDSTR& SystemInfo_GetCPUID(STDSTR* retStr){
	*retStr += "CUPID is TESTCPU001";
	return(*retStr);
};
#endif
//------------------------------------------------------------------------------------------//
