...
//------------------------------------------------------------------------------------------//
ExpandDevBusAttr* BIC_ENV_DEV::GetEDA(CMD_ENV* env){
#ifdef Bus_h
...
//------------------------------------------------------------------------------------------//
ExpandDevBusAttr* BIC_ENV_DEV::GetEDA_M(CMD_ENV* env){
#ifdef Bus_h
	return(CMD_ENV::GetVar(env,CMD_VID_EDA_M,(ExpandDevBusAttr*)nullptr));
#else
	return(nullptr);
#endif
};
//------------------------------------------------------------------------------------------//
ExpandDevBusAttr* BIC_ENV_DEV::GetEDA_A(CMD_ENV* env){
#ifdef Bus_h
	return(CMD_ENV::GetVar(env,CMD_VID_EDA_A,(ExpandDevBusAttr*)nullptr));
#else
	return(nullptr);
#endif
};
//------------------------------------------------------------------------------------------//
ExpandDevBusAttr* BIC_ENV_DEV::GetEDA_S(CMD_ENV* env){
#ifdef Bus_h
	return(CMD_ENV::GetVar(env,CMD_VID_EDA_S,(ExpandDevBusAttr*)nullptr));
#else
	return(nullptr);
#endif
};
...
//------------------------------------------------------------------------------------------//
bool32 BIC_CONNECT::SetConnectPar(CMD_ENV* env,const STDSTR &par,uint32 devType){
	ExpandDevBusAttr *eda = BIC_ENV_DEV::GetEDA(env);
	STDSTR	strPar1,strPar2;
	SplitPar1(strPar1,strPar2,par);
	if (eda->SetConnectPar(devType,strPar1,strPar2) != G_FALSE)
		return G_TRUE;
	PrintFail(env,"already connected");
	return G_FALSE;
};
//------------------------------------------------------------------------------------------//
bool32 BIC_CONNECT::SetConnectPar2(CMD_ENV* env,const STDSTR& par){
	ExpandDevBusAttr *eda = BIC_ENV_DEV::GetEDA(env);

	if (eda->SetConnectPar(eda->openType,"",par) != G_FALSE)
		return G_TRUE;
	if (eda->SetConnectBR(par) != G_FALSE)
		return G_TRUE;
	PrintFail(env,"already connected");
	return G_FALSE;
};
//------------------------------------------------------------------------------------------//
uint8 BIC_CONNECT::PressAnyKey(CMD_ENV* env,ExpandDevBusAttr* eda,SYS_TIME_S* dly){
	uint8	retChar;
	retChar = 0;
...
//------------------------------------------------------------------------------------------//
bool32 BIC_CONNECT::OnlineMode(CMD_ENV* env,ExpandDevBusAttr* attr,CMD_TAIL tail){
	uint8	chKey;
	...
	while((IsExit(env) == G_FALSE) && attr->IsOpened()){
		...
		if ((chKey >= 32) && (chKey <= 126)){
			...
			PrintEnable(env);
			PrintStr(env,"\n");
			attr->cdevbus->PrintSendStrWOG1(sendData + strTail + "\n");//for log
			attr->cdevbus->SendCommand(sendData,tail,G_ESCAPE_ON);
			sendData = "";
		}
		else if (chKey == 27){
			ret = G_TRUE;
			...
			if ((chKey > 0) && (chKey <= 31)){
				...
				strT = '^';
				strT += (chKey + 0x40);
				attr->cdevbus->PrintSendStrWOG1(strT);
				attr->cdevbus->Send(IUD(chKey),0);
			}
		}
	}
	PrintEnable(env);
...
//------------------------------------------------------------------------------------------//
void BIC_CONNECT::PrintTitle(CMD_ENV* env,ExpandDevBusAttr* eda,bool32 blPrintTail){
...
CMDID BIC_CR::Command(CMD_ENV* env,const STDSTR& msg,void* p)const{
	...
	PrintStrNL(env,COL_clCyan
			   , "Received :", Str_ToStr(attr->cdevbus->RxBytes()), "bytes\n"
			   , "Echo     :", Str_ToStr(attr->cdevbus->DPBytes()), "bytes\n"
			   , "Sent     :", Str_ToStr(attr->cdevbus->TxBytes()), "bytes\n"
...
CMDID BIC_ECHO::Command(CMD_ENV* env,const STDSTR& msg,void* p)const{
	...
	if (msg == "on"){
		attr->cdevbus->EnableEcho();
	}
	else if (msg == "off"){
		attr->cdevbus->DisableEcho();
	}
	else if(msg.length() > 0){
		return(Help(env,0));
	}
	
	if (attr->cdevbus->IsEnableEcho() == G_FALSE){
