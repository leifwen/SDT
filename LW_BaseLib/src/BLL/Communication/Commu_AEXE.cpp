...
CORE_AEXE::CORE_AEXE(void) : COMMU_CORE(){
	monitorThread.ThreadInit(this, &CORE_AEXE::MonitorThreadFunc,"monitor");
...
void CORE_AEXE::Init(const COMMU_TEAM* _team){
	...
	unitTeam->cdrv->ThreadAccept(&monitorThread);
};
//------------------------------------------------------------------------------------------//
...
bool32 CORE_AEXE::Execute(const STDSTR &name,const STDSTR& cmd){
	int32 ret;
	ret = 0;
	if (unitTeam->cdrv->cgOpenLock.Try() != G_FALSE){
		cgCommand = cmd;
		cgSH = "";
		ret = unitTeam->cdrv->Open(SetOpenPar(OPEN_None,name,0,0));
...
//------------------------------------------------------------------------------------------//
bool32 CORE_AEXE::MonitorThreadFunc(void* _team){
...
//------------------------------------------------------------------------------------------//
AEXEPOOL::AEXEPOOL(uint32 rxSize,uint32 txSize) : _AEXEPOOL(rxSize,txSize,nullptr){
...
//------------------------------------------------------------------------------------------//
void AEXEPOOL::CloseChild(COMMU_DRV *cdrv){
	_AEXEPOOL::CloseChild(cdrv);
};
//------------------------------------------------------------------------------------------//
