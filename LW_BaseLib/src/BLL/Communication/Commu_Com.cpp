...
CORE_ACOM::CORE_ACOM(void) : COMMU_CORE(){
	...
	modemStatusThread.ThreadInit(this, &CORE_ACOM::ModemStatusThreadFunc,"ms");
...
void CORE_ACOM::Init(const COMMU_TEAM* _team){
	...
	unitTeam->cdrv->ThreadAccept(&modemStatusThread);
};
//------------------------------------------------------------------------------------------//
#ifdef CommonDefH_VC
//------------------------------------------------------------------------------------------//
...
void CORE_ACOM::PrintOpenSuccess(const STDSTR& strTitle){
	...
	if (GetOpenPar().type == OPEN_COMV)
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,COL_NormalMessage,"Virtual COM",
									  COL_ImportantMessage,vPortName,
									  COL_NormalMessage,"has been created");
#endif
};
//------------------------------------------------------------------------------------------//
void CORE_ACOM::PrintClose(const uint64& rxBytes,const uint64& txBytes,const uint64& dpBytes){
	if (unitTeam->cdrv->IsCloseDueToOS()){
		COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"COM","connection disconnected");
	}
	else{
		COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"User closed","COM");
	}
#ifdef CommonDefH_Unix
	if (GetOpenPar().type == OPEN_COMV){
...
//------------------------------------------------------------------------------------------//
bool32 CORE_ACOM::ModemStatusThreadFunc(void* _team){
	...
	while(modemStatusThread.IsTerminated() == G_FALSE){
		...
		blflag = 0;
#ifdef ODEV_BUS_h
		if (COMMU_ODEVBUS::GetODevBus(odevBus) != nullptr)
			blflag = B_ChkFLAG64(COMMU_ODEVBUS::GetODevBus(unitTeam->odevBus)->envcfg,ODEV_FLAG_EnMSReport);
#endif
		if (blflag == 0){
			...
			if (strMSstatus != modemStatus){
				COMMU_ODEVBUS::PrintConnectionReport(odevBus,cdrv->RxBytes(),cdrv->TxBytes(),cdrv->DPBytes()
...
void CORE_ACOM::SetDTR(bool32 blHigh) {
	if (blHigh != G_FALSE){
		SetDTRToHigh();
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Set","DTR to high");
	}
	else{
		SetDTRToLow();
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Set","DTR to low");
	}
};
//------------------------------------------------------------------------------------------//
void CORE_ACOM::SetRTS(bool32 blHigh) {
	if (blHigh != G_FALSE){
		SetRTSToHigh();
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Set","RTS to high");
	}
	else{
		SetRTSToLow();
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Set","RTS to low");
	}
};
//------------------------------------------------------------------------------------------//
