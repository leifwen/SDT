...
MSG_CENTER::MSG_CENTER(void) : MSG_BASE(){
	...
	cgENV = nullptr;
	cgTeam = nullptr;
	t1CtrlRxThread.ThreadInit(this,&MSG_CENTER::T1CtrlRxThreadFunc	,"T1CRx");
...
#ifdef Commu_SSL_h
	_GetTeam(_team)->cdrv->ThreadAccept(&t1CtrlRxThread);
	cgTeam = _team;
	cgENV = env;
#endif
};
...
//------------------------------------------------------------------------------------------//
bool32 MSG_CENTER::T1CtrlRxThreadFunc(void* _team){
