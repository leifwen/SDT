...
MSG_Terminal::MSG_Terminal(void) : MSG_NODE(){
	...
	BICThread.ThreadInit(this, &MSG_Terminal::BICThreadFunc, "BICThread");
...
void MSG_Terminal::Reset(void* _team){
	...
	thread = cdrv->ThreadGet("BICThread");
	TNF::Disable(thread);
...
//------------------------------------------------------------------------------------------//
bool32 MSG_Terminal::BICThreadFunc(void* _team){
#ifdef Commu_Base_h
	COMMU_TEAM*		team = static_cast<COMMU_TEAM*>(_team);
	COMMU_DRV*		cdrv = team->cdrv;
	SYS_AThread*	thread = nullptr;
	...
	if (thread != nullptr){
		...
		MSGSend(MESG_ANS_SetupTerminal,IUD("Y"),blRet,"BICThreadFunc","Send");
		
		thread->Execute(_team);
	
		MSGSend(MESG_ANS_CloseTerminal,IUD(""),blRet,"BICThreadFunc","Send");
	}
	else{
		MSGSend(MESG_ANS_SetupTerminal,IUD("N"),blRet,"BICThreadFunc","Send");
	}
	ClrSFlag(TER_blSetupTerminalY | TER_blSetupTerminalN | TER_blCloseTerminalY);
