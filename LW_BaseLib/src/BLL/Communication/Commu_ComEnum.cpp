...
void IPCOMNAME::CreateShowName(void){
	...
	rwLock.W_set();
	strShowName = "";
	if (typeID == COMENUM_DEVID_APICOM){
	...
	if (typeID == COMENUM_DEVID_TCPClient){
	...
	if (typeID == COMENUM_DEVID_UDPClient){
		if (strUserDefineName.length() == 0){
			strShowName = "UDP  ::  ";
			strShowName += strIPComName;
			strShowName += "  :  ";
	...
	if (typeID == COMENUM_DEVID_TCPServer){
		if (strUserDefineName.length() == 0){
	...
	if (typeID == COMENUM_DEVID_UDPServer){
		if (strUserDefineName.length() == 0){
...
bool32 NewNodeCheck(IPCOMNAME* nodeInList,IPCOMNAME* checkNode){
	...
	if ((nodeInList->typeID == checkNode->typeID) && (Str_UpperCase(nodeInList->strIPComName) == Str_UpperCase(checkNode->strIPComName))){
		if (checkNode->typeID == COMENUM_DEVID_APICOM){
...
void IPCOMLIST::Refresh(void){
	...
	while (i++ < num){
		node.Init();
		node.typeID = COMENUM_DEVID_APICOM;
...
void IPCOMLIST::ExportCOM(IPCOMLIST* cExportList){
	...
	TREE_DownChain_Traversal_LINE(IPCOMNAME,this,
		_opNode->rwLock.R_set();
		if ((_opNode->blAvailable != 0) && (_opNode->typeID == COMENUM_DEVID_APICOM))
			cExportList->AddWithCheck(_opNode,G_LOCK_OFF);
		_opNode->rwLock.R_clr();
	);
};
