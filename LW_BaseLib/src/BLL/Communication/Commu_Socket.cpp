...
bool32 CORE_SOCKET::OpenDev_server(const OPEN_PAR& par){
	SetTypePar(par.type);
#ifdef ODEV_BUS_h
	COMMU_ODEVBUS::CreateODev_G3File(unitTeam->odevBus,ODEV_FILE::CreateNewLOGFileName(par.name, par.port));
#endif
	SetSocketBufferSize();
...
void CORE_SOCKET::CloseDev_server(void){
	...
	node = COMMU_ODEVBUS::GetG3DefFile(unitTeam->odevBus);
	if (node != nullptr)
		node->RemoveSelf();
	osHandle = INVALID_SOCKET;
};
...
bool32 CORE_SOCKET::Open(const SOCKET& sID,const sockaddr_in& addr,uint32 type,uint32 cfg){
	...
	osHandle = sID;
	return(unitTeam->cdrv->Open(SetOpenPar(type,strIP,port,cfg)));
};
//------------------------------------------------------------------------------------------//
...
void CORE_SOCKET::PrintOpenSuccess(const STDSTR& strTitle){
	switch (GetOpenPar().type) {
		...
		case OPEN_TCPS:;
		case OPEN_UDPS:;
			COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Client",this->GetDevName(),"connected");
			break;
		default:
			break;
	}
...
//------------------------------------------------------------------------------------------//
void CORE_SOCKET::PrintClose(const uint64& rxBytes,const uint64& txBytes,const uint64& dpBytes){
	if (unitTeam->cdrv->IsCloseDueToOS()){
		switch (GetOpenPar().type) {
			case OPEN_TCP:
				COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"TCPS",GetDevName(),"connection disconnected");
				break;
			case OPEN_UDP:
				COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"UDPS",GetDevName(),"connection disconnected");
				break;
			case OPEN_TCPS:
				COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"Client TCP",GetDevName(),"disconnected");
				break;
			case OPEN_UDPS:
				COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"Client UDP",GetDevName(),"disconnected");
			default:
				break;
		}
	}
		...
		switch (GetOpenPar().type) {
			case OPEN_TCP:
				COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"User closed TCP connection");
				break;
			case OPEN_UDP:
				COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"User closed UDP connection");
				break;
			case OPEN_TCPS:
				COMMU_ODEVBUS::PrintConnectionReport(unitTeam->odevBus,rxBytes,txBytes,dpBytes,"Client TCP",GetDevName(),"disconnected");
				break;
...
typedef COMMU_DRV_T<0,COMMU_DRV,CORE_SOCKET> S_SOCKET;
//------------------------------------------------------------------------------------------//
CORE_SOCKETSERVER::CORE_SOCKETSERVER(void) : COMMU_CORE(){
	listionSocket = INVALID_SOCKET;
	listionThread.ThreadInit(this,&CORE_SOCKETSERVER::ListionThreadFunc,"listion");
...
void CORE_SOCKETSERVER::Init(const COMMU_TEAM* _team){
	...
	unitTeam->cdrv->ThreadAccept(&listionThread);
};
//------------------------------------------------------------------------------------------//
...
//------------------------------------------------------------------------------------------//
bool32 CORE_SOCKETSERVER::ListionThreadFunc(void* _team){
	if (GetOpenPar().type == OPEN_TCPS){
...
bool32 CORE_SOCKETSERVER::ListionTCP(void* _team){
	...
	while(listionThread.IsTerminated() == G_FALSE){
		newSocket = static_cast<S_SOCKET*>(cdrv->GetNewNode());
		sID = accept(listionSocket,(struct sockaddr *)&ListionAddr,&addrlen);
		if (sID != INVALID_SOCKET){
			if (newSocket == nullptr){
				CloseSocket(sID);
				COMMU_ODEVBUS:: PrintWarningMessageDot(team->odevBus,"No enough resource to create new socket");
			}
			else{
				cdrv->AppendChild(newSocket);
...
bool32 CORE_SOCKETSERVER::ListionUDP(void* _team){
	...
	while(listionThread.IsTerminated() == G_FALSE){
		...
		if (bytesNum > 0){
			...
			udpSocket = static_cast<S_SOCKET*>(cdrv->Find(strIP,port));
...
void CORE_SOCKETSERVER::PrintOpenSuccess(const STDSTR& strTitle){
	if (GetOpenPar().type == OPEN_TCPS){
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Successfully setup TCP Server, listion Port is", Str_ToStr(GetOpenPar().port));
	}
	else{
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Successfully setup UDP Server, listion Port is", Str_ToStr(GetOpenPar().port));
	}
};
//------------------------------------------------------------------------------------------//
void CORE_SOCKETSERVER::PrintOpenFail(const STDSTR& strTitle){
	if (GetOpenPar().type == OPEN_TCPS){
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Failed setup TCP Server,", "please try again");
	}
	else{
		COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"Failed setup UDP Server,", "please try again");
	}
};
//------------------------------------------------------------------------------------------//
void CORE_SOCKETSERVER::PrintClose(const uint64& rxBytes,const uint64& txBytes,const uint64& dpBytes){
	COMMU_ODEVBUS::PrintMessageDot(unitTeam->odevBus,"User stopped the server");
};
//------------------------------------------------------------------------------------------//
bool32 CORE_SOCKETSERVER::OnOpenTCPSocket	(COMMU_DRV* newSocket)	{return G_TRUE;};
bool32 CORE_SOCKETSERVER::OnOpenUDPSocket	(COMMU_DRV* newSocket)	{return(OnOpenTCPSocket(newSocket));};
//------------------------------------------------------------------------------------------//
#endif /* Commu_Com_h */
