...
//------------------------------------------------------------------------------------------//
TNF* TNF::FindInDownChainByUniqueID(TNF *tnNode, uint32 tUniqueID){
	TNF	*ret;
	ret = nullptr;
	if (tnNode != nullptr){
		TREE_DownChain_Find(TNF,tnNode,ret,(GetUniqueID(_opNode) == tUniqueID));
	}
	return(ret);
};
//------------------------------------------------------------------------------------------//
...
//------------------------------------------------------------------------------------------//
TNFP* TNFP::FindInDownChainBySelfName(TNFP* tnfpNode,const STDSTR& strName){
	TNFP	*ret;
	ret = nullptr;
	if (tnfpNode != nullptr){
		TREE_DownChain_Find(TNFP,tnfpNode,ret,(_opNode->selfName == strName));
	}
	return(ret);
};
//------------------------------------------------------------------------------------------//
