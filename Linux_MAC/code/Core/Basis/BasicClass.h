...
class BASIC_CFLAG{
	...
	private:
		SYS_Lock	cgInUse;
	public:
		inline void		Spin_InUse_set(G_LOCK_VAILD blVaild = G_LOCK_ON){cgInUse.Lock(blVaild);};
		inline void		Spin_InUse_clr(G_LOCK_VAILD blVaild = G_LOCK_ON){cgInUse.Unlock(blVaild);};
		inline int32	Spin_InUse_try(G_LOCK_VAILD blVaild = G_LOCK_ON){return(cgInUse.TryLock(blVaild));};
	protected:
