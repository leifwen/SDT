...
//------------------------------------------------------------------------------------------//
const std::string & MD5Sum::Calculate(const std::string &sSource){
	return(Calculate((uint8 *)sSource.c_str(),(uint32)sSource.length()));
}
//------------------------------------------------------------------------------------------//
const std::string & MD5Sum::Calculate(const uint8* pachSource,uint32 nLen){
	MD5_CTX context;
	...
	m_sHash = Str_CharToHEX(m_rawHash, sizeof(m_rawHash),G_SPACE_OFF);
	return(m_sHash);
}
//------------------------------------------------------------------------------------------//
const std::string & MD5Sum::CalculateFile(const std::string &tFileName){
	uint8			buffer[8192];
...
static void MD5Transform  (UINT4 [4], const uint8 [64]);
static void Encode        (uint8 *, const UINT4 *, uint32);
static void Decode        (UINT4 *, const uint8 *, uint32);
...
static void MD5Transform (UINT4 state[4], const uint8 block[64]){
...
static void Encode (uint8 *output, const UINT4 *input, uint32 len){
	uint32 i, j;
...
static void Decode (UINT4 *output, const uint8 *input, uint32 len){
	uint32 i, j;
