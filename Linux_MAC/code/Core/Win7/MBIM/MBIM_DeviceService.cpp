...
//------------------------------------------------------------------------------------------//
int32 MBIM_DevService::OpenV(const std::string &tCDBufName, int32 tCDBufPar, CSType tCSType, int32 blEnEcho){
	if (IsConnected() > 0)
		return 1;
	return 1;
}
	...
	*retNum = 0;
	...
	if (ComStat.cbInQue > 0){
		...
		if (length > dwBytesRead)
			length = dwBytesRead;
		if (ReadFile(Handle,buffer,length,&dwBytesRead,NULL)){
